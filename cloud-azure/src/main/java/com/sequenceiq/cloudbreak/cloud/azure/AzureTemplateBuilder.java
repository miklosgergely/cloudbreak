...
import com.sequenceiq.cloudbreak.cloud.azure.view.AzureDatabaseView;
...
public class AzureTemplateBuilder {
    ...
    public String build(String stackName, CloudContext cloudContext, DatabaseStack databaseStack) {
        try {
            ...
            AzureDatabaseView azureDatabaseView = new AzureDatabaseView(databaseStack.getDatabaseServer());
            Map<String, Object> model = new HashMap<>();

            model.put("adminLoginName", azureDatabaseView.getAdminLoginName());
            model.put("adminPassword", azureDatabaseView.getAdminPassword());
            model.put("backupRetentionDays", azureDatabaseView.getBackupRetentionDays());
            model.put("dbServerName", azureDatabaseView.getDbServerName());
            model.put("dbVersion", azureDatabaseView.getDbVersion());
            model.put("geoRedundantBackup", azureDatabaseView.getGeoRedundantBackup());
            model.put("location", location);
            model.put("serverTags", databaseStack.getTags());
            model.put("skuCapacity", azureDatabaseView.getSkuCapacity().toString());
            model.put("skuFamily", azureDatabaseView.getSkuFamily());
            model.put("skuName", azureDatabaseView.getSkuName());
            model.put("skuSizeMB", azureDatabaseView.getAllocatedStorageInMb().toString());
            model.put("skuTier", azureDatabaseView.getSkuTier());
            model.put("storageAutoGrow", azureDatabaseView.getStorageAutoGrow());
